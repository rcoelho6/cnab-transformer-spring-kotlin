...
com.reicorp.cnab.transformer_spring_kotlin package structure

Basic 4-layer structure

```mermaid
graph TD
  Root[com.reicorp.cnab.transformer_spring_kotlin]
...
- Packages:
  - `com.reicorp.cnab.transformer_spring_kotlin.domain.entities` — domain entities
  - `com.reicorp.cnab.transformer_spring_kotlin.domain.services` — domain services
  - `com.reicorp.cnab.transformer_spring_kotlin.domain.ports.in` — inbound contracts defined by the core
  - `com.reicorp.cnab.transformer_spring_kotlin.domain.ports.out` — outbound contracts defined by the core
...
- Packages:
  - `com.reicorp.cnab.transformer_spring_kotlin.technical_core.mappers` — mapping logic (CNAB <-> JSON), bank-specific strategies/versioning
  - `com.reicorp.cnab.transformer_spring_kotlin.technical_core.mq` — MQ helpers (enqueue/dequeue patterns)
  - `com.reicorp.cnab.transformer_spring_kotlin.technical_core.formatters` — formatting utilities (dates, numbers, codecs)
...
- Packages:
  - `com.reicorp.cnab.transformer_spring_kotlin.application.controllers` — request handlers / controllers
  - `com.reicorp.cnab.transformer_spring_kotlin.application.services` — application services / orchestrators
  - `com.reicorp.cnab.transformer_spring_kotlin.application.dto` — DTOs used at the application boundary
...
- Packages (under `infrastructure`):
  - `com.reicorp.cnab.transformer_spring_kotlin.infrastructure.rest` — REST controllers and REST clients
  - `com.reicorp.cnab.transformer_spring_kotlin.infrastructure.jms` — JMS listeners and producers
  - `com.reicorp.cnab.transformer_spring_kotlin.infrastructure.repositories` — repository implementations (DAOs)
  - `com.reicorp.cnab.transformer_spring_kotlin.infrastructure.repositories.models` — persistence models (DB/JPA entities)
